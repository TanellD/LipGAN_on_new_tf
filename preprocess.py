...
def dump_split(args):
	...
	output_directory = "data_root/main/s1_preprocessed"
	
	if not os.path.exists(output_directory):
		os.makedirs(output_directory)
	filelist = []
	for input_image in os.listdir("data_root/main/s1"):
		output_image = input_image[:-1]
		input_image = "data_root/main/s1/" + input_image
		output_image += "4"
		output_image = path.join(output_directory, output_image)
		ffmpeg.input(input_image).output(output_image).run()
		filelist.append(output_image)

	jobs = [(vfile, args, ('image_frames')) for vfile in filelist]
